...
void bottle_vm_init(bottle_vm_t* vm, const bottle_program_t* program) {
  ...
  // Initialize arrays based on element type
  for (uint8_t i = 0; i < program->array_count; i++) {
    const bottle_array_def_t* def = &program->arrays[i];
    if (def->element_type == BOTTLE_TYPE_FLOAT) {
      for (uint8_t j = 0; j < def->length && j < MATRIX_WIDTH; j++) {
        vm->float_arrays[i][j] = def->initial_value.float_value;
      }
    } else {
      for (uint8_t j = 0; j < def->length && j < MATRIX_WIDTH; j++) {
        vm->int_arrays[i][j] = def->initial_value.int_value;
      }
    }
  }

  // Copy array type metadata
  for (uint8_t i = 0; i < program->array_count; i++) {
    vm->array_types[i] = program->arrays[i].element_type;
  }
  ...
  // Execute array literal initialization instructions at offset 0
  // These are generated by state declarations with array literals
  vm->pc = 0;
  while (vm->pc < program->bytecode_size) {
    uint8_t opcode = program->bytecode[vm->pc];

    if (opcode == OP_INIT_ARRAY_LITERAL) {
      vm->pc++; // Skip opcode
      uint8_t array_idx = read_byte(program, &vm->pc);
      uint8_t count = read_byte(program, &vm->pc);

      for (uint8_t i = 0; i < count && i < program->arrays[array_idx].length; i++) {
        uint16_t const_idx = read_u16(program, &vm->pc);
        if (const_idx >= program->constant_count) continue;

        bottle_value_t val = program->constants[const_idx];

        if (vm->array_types[array_idx] == BOTTLE_TYPE_FLOAT) {
          vm->float_arrays[array_idx][i] = bottle_to_float(val);
        } else {
          vm->int_arrays[array_idx][i] = bottle_to_uint8(val);
        }
      }
    } else {
      // Stop when we hit a non-initialization instruction
      break;
    }
  }

  // Reset PC for normal execution
  vm->pc = 0;
}
  ...
  while (vm->pc < program->bytecode_size) {
    ...
    switch ((bottle_opcode_t)opcode) {
      ...
      case OP_PUSH_ARRAY: {
        ...
        // Push value based on array element type
        if (vm->array_types[array_idx] == BOTTLE_TYPE_FLOAT) {
          push(vm, bottle_float(vm->float_arrays[array_idx][index]));
        } else {
          push(vm, bottle_int(vm->int_arrays[array_idx][index]));
        }
        break;
      }
      ...
      case OP_POP_ARRAY: {
        ...
        // Store value based on array element type
        if (vm->array_types[array_idx] == BOTTLE_TYPE_FLOAT) {
          vm->float_arrays[array_idx][index] = bottle_to_float(value);
        } else {
          vm->int_arrays[array_idx][index] = bottle_to_uint8(value);
        }
        break;
      }
      ...
      case OP_READ_SPECTRUM: {
        ...
        // Read spectrum data into array (always writes to int_arrays since spectrum is uint8_t)
        uint8_t count = min(program->arrays[array_idx].length, (uint8_t)MATRIX_WIDTH);
        for (uint8_t i = 0; i < count; i++) {
          if (vm->orientation & 1) {
            // Rotated: average two adjacent bands
            vm->int_arrays[array_idx][i] = (ctx->sensor.spectrum[i * 2] +
                                            ctx->sensor.spectrum[i * 2 + 1]) / 2;
          } else {
            vm->int_arrays[array_idx][i] = ctx->sensor.spectrum[i];
          }
        }
        break;
      }
      ...
      case OP_INIT_ARRAY_LITERAL: {
        ...
        // Read constant indices and initialize array
        for (uint8_t i = 0; i < count && i < program->arrays[array_idx].length; i++) {
          uint16_t const_idx = read_u16(program, &vm->pc);
          ...
          bottle_value_t value = program->constants[const_idx];
          if (vm->array_types[array_idx] == BOTTLE_TYPE_FLOAT) {
            vm->float_arrays[array_idx][i] = bottle_to_float(value);
          } else {
            vm->int_arrays[array_idx][i] = bottle_to_uint8(value);
          }
        }
        break;
      }
