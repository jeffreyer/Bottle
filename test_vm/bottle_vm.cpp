  ...
  while (vm->pc < program->bytecode_size) {
    ...
    switch ((bottle_opcode_t)opcode) {
      ...
      case OP_POP_SCALAR: {
        ...
        static int pop_scalar_count = 0;
        if (pop_scalar_count < 20) {
          Serial.printf("[VM] OP_POP_SCALAR: scalar[%d] = %.1f\n", index, bottle_to_float(value));
          pop_scalar_count++;
        }
      ...
      case OP_LT: {
        ...
        if (lt_count < 10) {
          Serial.printf("[VM] OP_LT: %.1f < %.1f = %s\n",
                       bottle_to_float(a), bottle_to_float(b),
                       bottle_is_truthy(compare_lt(a, b)) ? "true" : "false");
          lt_count++;
        }
      ...
      case OP_JUMP: {
        ...
        if (jump_count < 5) {
          Serial.printf("[VM] OP_JUMP: pc_before=%d, offset=%d, pc_after=%d\n",
      ...
      case OP_SET_LED: {
        ...
        static int set_led_count = 0;
        if (set_led_count < 5) {
          Serial.printf("[VM] OP_SET_LED #%d: x_val(type=%d,int=%d) y_val(type=%d,int=%d) color_type=%d\n",
                       set_led_count, x_val.type, bottle_to_int(x_val),
                       y_val.type, bottle_to_int(y_val), color_val.type);
          set_led_count++;
        }
        ...
        if (map_debug < 3) {
          Serial.printf("[VM] Before map_xy: x=%d, y=%d, orientation=%d\n", x, y, vm->orientation);
          map_debug++;
        }
        ...
        static int map_debug2 = 0;
        if (map_debug2 < 3) {
          Serial.printf("[VM] After map_xy: px=%d, py=%d\n", px, py);
          map_debug2++;
        }
