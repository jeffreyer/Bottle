...
### 硬件能力
- **LED 矩阵**: 17×8 / 32x8 全彩 LED 阵列，支持动态动画
- **重力感应**: LIS3DH 三轴加速度计，支持姿态检测和重力交互
- **音频采集**: 支持 I2S/PDM 麦克风，实时 FFT 频谱分析（17 / 32 频段）
...
- 加速度计: LIS3DH
- LED: WS2812B 阵列
- 麦克风: I2S/PDM 数字麦克风（板子支持MSM261D3526Z1CM，MSM261S3526Z0CM，可通过串口命令 mic=1/0 切换）
...
### 编译和上传
- 当前有长短版两个版本，编译前需要修改common.h文件，修改第21行，BOTTLE_V4为长版32\*8像素，注释掉为短版17\*8像素，根据需要选择是否注释
```bash
# 编译固件
pio run
...
- **Lua模拟器**: [ESP32 Lua LED Matrix Simulator 使用说明](tools/lua-simulator/README.md)
...
1. 长按触摸键 2-3 秒待 LED 矩阵显示蓝牙图标进入 BLE 配置模式
...
1. 编写 Lua 脚本和配置项（参考 [Lua 开发指南](docs/LUA_SCRIPT_GUIDE.md)）
2. 放入U盘扩展名*.lua(代码文件),*.cfg(配置项文件)
3. 无需重新编译固件
